...
def find_pdf_links(soup, base_url):
    ...
    for iframe in iframes:
        ...
        if src and '.pdf' in src.lower():
            if src.startswith('/'):
                src = 'https://www.cug.edu.cn' + src
            pdf_links.append({'name': 'iframe_pdf', 'url': src})
...
def download_pdf(pdf_url, save_dir='pdf_cache'):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            safe_print(f"  PDF下载失败: HTTP {response.status_code}")
            return None
    except Exception as e:
        safe_print(f"  PDF下载异常: {e}")
        return None
...
def extract_pdf_text(pdf_path):
    ...
    try:
        ...
        with open(pdf_path, 'rb') as f:
            ...
            for page_num, page in enumerate(reader.pages, 1):
                try:
                    ...
                    safe_print(f"  提取第{page_num}页失败: {e}")
                    continue

    except ImportError:
        safe_print("  提示: 未安装PyPDF2，正在尝试安装...")
        ...
        try:
            ...
            safe_print("  PyPDF2安装失败，尝试pdfplumber...")
            try:
                import pdfplumber
                ...
                safe_print("  提示: 未安装pdfplumber，请运行: pip install pdfplumber")
                return ""
...
def extract_pdf_images(pdf_path):
    ...
    try:
        ...
        safe_print("  提示: 未安装PyMuPDF，请运行: pip install pymupdf")
