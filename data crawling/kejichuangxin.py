...
def get_news_list(page_num):
    """获取某一页的新闻列表"""
    if page_num == 1:
        url = "https://www.cug.edu.cn/index/kjcx.htm"
    else:
        # 分页URL格式（需要测试是否也是反向的）
        url = f"https://www.cug.edu.cn/index/kjcx/{page_num}.htm"
...
def save_to_csv(all_news, filename="kejichuangxin.CSV"):
    """保存数据到CSV文件"""
...
def main():
    ...
    print(f"开始爬取中国地质大学'科技创新'栏目，共{total_pages}页...")
    print("=" * 60)
    ...
    for page in range(1, total_pages + 1):
        ...
        if len(news_list) == 0 and page > 1:
            print(f"  尝试反向分页...")
            page_value = 21 - page + 1
            url = f"https://www.cug.edu.cn/index/kjcx/{page_value}.htm"
            try:
